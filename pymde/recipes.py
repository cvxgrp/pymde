...
def preserve_distances(
    ...
    distances:
        ...
        A pymde.Graph instance is interpreted as encoding all (n_items choose
        2) distances: the distance between i and j is taken to be the length of
        the shortest path connecting i and j.
    ...
    ---------
        ...
        constraint: pymde.constraints.Constraint (optional)
            Embedding constraint, pymde.Standardized() or
            ...
            distance can be).
        max_distances: int
            Max number of distances to compute.
        device: str (optional)
...
def preserve_neighbors(
    ...
    generate features for machine learning tasks (with embedding_dim=10, 50,
    or 100, for example). It typically yields embeddings in which similar items
    ...
    matrices; graphs should be instances of pymde.preprocess.Graph.

    The MDE problem uses distortion functions derived from weights.
    ...
    ---------
        ...
        constraint: pymde.constraints.Constraint (optional)
            ...
            pymde.Anchored(anchors, values) (or None). Defaults no constraint
            when a repulsive penalty is provided, otherwise defaults to
            pymde.Standardized().
